...
const age6 = [
  ...
  calcAge2(yearss[1]),
  calcAge2(yearss[2]),
  calcAge2(yearss[yearss.length - 1]),
];
...
const jonas1 = {
  ...
  getSumary: function () {
    return `${this.firstName} is a ${this.calcAge11()}-year old ${
      this.job
    }, and he ${this.hasDriversLicenses ? "has" : "no have"} a driver license.`;
  },
};
...
const bmiObj = {
  ...
  higherBmi: function () {
    if (this.calcBmi() > this.calcBmi1()) {
      return `Mark weights is ${this.calcBmi().toFixed(
        2
      )} greater than john's is ${this.calcBmi1().toFixed(2)} `;
    } else {
      return `John weights is ${this.calcBmi1().toFixed(
        2
      )} greater than Mark is ${this.calcBmi().toFixed(2)} `;
    }
  },
};
