...
*/
function SelectStmt(input, index) {
...
*/
function ClauseFrom(input, index) {
...
*/
function ClauseWhere(input, index) {
...
*/
function Condition(input, index) {
  const parts = [];
...
*/
function TableName(input, index) {
...
*/
function ColumnName(input, index) {
...
function parse(input) {
  ...
  if (result.accepted) {
    if (result.lastIndex === input.length) {
      return Object.assign(Object.assign({}, result), { accepted: true });
    }
...
const input = 'SELECT column2 FROM table2 WHERE column1 = 34';
if (input) {
